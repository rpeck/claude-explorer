...
def test_install_all_runs_watcher_and_mcp(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cm, "_do_scheduled_fetch", lambda *a, **k: seen.append("fetch") or _ok("fetch"))
    ...
    assert seen == ["watcher", "fetch", "code", "desktop"]
...
def test_install_all_continues_and_exits_one_on_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cm, "_do_scheduled_fetch", lambda *a, **k: _ok("fetch"))
...
def test_install_all_uninstall(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cm, "_do_scheduled_fetch", lambda iv, un: seen.append(("fetch", un)) or _ok("fetch"))
    ...
    assert res.exit_code == 0
    assert seen == [("watcher", True), ("fetch", True), "u-code", "u-desktop"]
